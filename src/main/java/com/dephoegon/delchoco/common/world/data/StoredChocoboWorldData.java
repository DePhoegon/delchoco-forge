package com.dephoegon.delchoco.common.world.data;
...
public class StoredChocoboWorldData extends SavedData
{
	private static final String name = DelChoco.DELCHOCO_ID + "_stored_horses";

	private final Map<String, Integer> entries = new HashMap<>();
	private final List<String> killedHorses = new ArrayList<>();
	private final List<String> disbandedHorses = new ArrayList<>();
	private final Map<String, CompoundTag> offlineSavedHorses = new HashMap<>();

	private int i = 0;

	public StoredChocoboWorldData()
	{

	}

	public static StoredChocoboWorldData load(CompoundTag nbt)
	{
		StoredChocoboWorldData data = new StoredChocoboWorldData();
		int i = 0;
	...
	public static StoredChocoboWorldData getInstance(Level level)
	{
		...
		return storage.computeIfAbsent(StoredChocoboWorldData::load, StoredChocoboWorldData::new, name);
	}
}
