...
public class ChocoboInventoryScreen extends AbstractContainerScreen<SaddleBagContainer> {
    ...
    protected void renderBg(@NotNull PoseStack matrixStack, float partialTicks, int x, int y) {
        ...
        if(!saddleStack.isEmpty()){
            ...
            if(item == ModRegistry.CHOCOBO_SADDLE.get()) { RenderSystem.setShaderTexture(0, INV_TEXTURE_NULL); }
            else if(item == ModRegistry.CHOCOBO_SADDLE_BAGS.get()) { RenderSystem.setShaderTexture(0, INV_TEXTURE_SMALL); }
            else if(item == ModRegistry.CHOCOBO_SADDLE_PACK.get()) { RenderSystem.setShaderTexture(0, INV_TEXTURE_LARGE); }
